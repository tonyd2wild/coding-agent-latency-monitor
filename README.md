...
- **🖥️ Live Fleet** — an always-on, command-center-dense view of your **whole rig** at once, polled in parallel (starts on page load, not just during a run). Each device gets a compact header (name + reachable dot + jump badge) followed by **one mini-module per GPU** (GPU 0, GPU 1, … each its own card with an animated util bar, a colored temp pill, and VRAM used/total), plus system RAM. Add an optional **`"group"`** to lay several devices out together: every device sharing a group value renders as **one full-width horizontal row** of GPU modules — a multi-GPU box (e.g. a quad-3090) spreads its four GPUs across the row, while a set of single-GPU boxes (e.g. four DGX Sparks) each contribute a module tagged with the device name (`Bluey · GPU0`) so you can tell them apart. Devices with no `group` render standalone as before. Devices can be **direct** (`ssh -i key user@host`) or reached through a **nested bastion** (`"jump": "jumpuser@jumphost"`): the app ssh's into the jump host, which then reaches the inner node with its own LAN access, so you only need to reach the bastion (ideal for Tailscale-gated or LAN-only cluster nodes). An optional **fabric-switch module** (RouterOS/MikroTik over SSH) shows switch/CPU temps and a dense row of active fabric ports with live Tx/Rx and link speed. Unreachable devices/GPUs/switch dim gracefully as *offline* instead of erroring. Config-driven via `fleet.json` (see below). Backed by `GET /api/fleet` (parallel SSH with short timeouts + a server-side cache so rapid polls never spam SSH; the switch is refreshed on a slow background cadence so a slow jump-host round-trip never blocks the view).
- **🧠 Live Models** — a big live card per model that's actually up, spread across one full-width row: it probes each unique preset endpoint's `/v1/models`, and for the ones responding shows the model name, endpoint host, live **requests running / waiting**, **KV-cache %** (with an animated bar), and an **up/idle status dot** (busy when there are running requests). Shows *No models running* when nothing answers. Same `/api/fleet` poll.
...
```json
{
  ...
  "devices": [
    { "name": "GPU Rig A", "user": "youruser", "host": "192.0.2.10", "ssh_key": "~/.ssh/id_ed25519", "group": "GPU Rig A", "temp_warn": 70, "temp_hot": 84 },
    { "name": "Spark 1", "user": "youruser", "host": "192.0.2.21", "group": "DGX Sparks", "jump": "bastionuser@bastion.example.local" },
    { "name": "Spark 2", "user": "youruser", "host": "192.0.2.22", "group": "DGX Sparks" }
  ],
  "switch": {
...
```
- **`devices[]`** — `name` (card label), `user` + `host` (→ `user@host`), optional `ssh_key` (falls back to `ssh.default_key`), optional `group` (string; devices sharing a value render together as one full-width row — see Live Fleet above), optional `jump` = `"jumpuser@jumphost"` (adds `ssh -J …` ProxyJump for bastion-only boxes), optional `port`, optional `temp_warn`/`temp_hot` (°C for the GPU temp pills). Each device renders **one module per GPU**.
